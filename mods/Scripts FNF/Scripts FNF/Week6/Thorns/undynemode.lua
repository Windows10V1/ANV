...
function onCreate()
	...
	if difficulty == 1 then
		...
		function onUpdate(elapsed)
			for i = 0,3 do
				setPropertyFromGroup('opponentStrums', i, 'texture', 'undynemode/NOTE_assets')
			end
		
			if getPropertyFromGroup('unspawnNotes', i, 'mustPress') == false then
				setPropertyFromGroup('unspawnNotes', i, 'texture', 'undynemode/NOTE_assets');
			end
			noteTweenX('dadleftx', 0, 400, 0.1, 'sineInOut');
			noteTweenX('daddownx', 1, 525, 0.1, 'sineInOut');
			noteTweenX('dadupx', 2, 650, 0.1, 'sineInOut');
			noteTweenX('dadrightx', 3, 775, 0.1, 'sineInOut');

			noteTweenY('dadlefty', 0, 750, 0.1, 'sineInOut');
			noteTweenY('daddowny', 1, 750, 0.1, 'sineInOut');
			noteTweenY('dadupy', 2, 750, 0.1, 'sineInOut');
			noteTweenY('dadrighty', 3, 750, 0.1, 'sineInOut');

			setPropertyFromGroup('strumLineNotes', 0, 'alpha', 0.2)
			setPropertyFromGroup('strumLineNotes', 1, 'alpha', 0.2)
			setPropertyFromGroup('strumLineNotes', 2, 'alpha', 0.2)
			setPropertyFromGroup('strumLineNotes', 3, 'alpha', 0.2)
		end
	end
end
...
-- checklist
-- [] fix the heart icon weird idle bug thingamabob
-- [] strupid shield thing wont return to blue most of the times
-- [] HP health bar
-- [] start at MAX HP 20/20, miss you get 4 DMG, hit you get 1 HP
-- [] HP bar at bottom right
-- [] score, misses, judgements like side hud at top left
-- [] box animation at the start? maybe?
-- [] force notes to start at the middle, not go there weirdly
-- [] yellow. fucking. notes
-- [] notes got weird again (successful press always looking down)